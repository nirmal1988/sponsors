...
var AppComponent = /** @class */ (function () {
    ...
    AppComponent.prototype.scrollInitially = function () {
        var _this = this;
        setTimeout(function () {
            _this.animate(document.scrollingElement || document.documentElement, "scrollTop", "", 0, 0, 0, true);
        }, 500);
    };
    AppComponent.prototype.swithchMenu = function () {
        var _this = this;
        setTimeout(function () {
            var selectedMenu = document.getElementsByClassName('mat-tab-label-active')[0];
            console.log(selectedMenu);
            console.log('next---', selectedMenu.nextElementSibling);
            if (selectedMenu.nextElementSibling) {
                var scrollFrom = 0;
                if (_this.prevScrollEle)
                    scrollFrom = _this.prevScrollEle.offsetTop;
                _this.scrollInitially();
                _this.prevScrollEle = null;
                _this.pageNumber = 0;
                _this._menuId = _this._menuId + 1;
                _this.changeInterval();
                _this.doNotScroll = true;
                setTimeout(function () {
                    _this.doNotScroll = false;
                }, (_this.scrollInterval - 2000));
                var nextEle = selectedMenu.nextElementSibling;
                nextEle.click();
            }
            else {
                console.log('reload');
    ...
    AppComponent.prototype.startPaging = function () {
        var _this = this;
        setTimeout(function () {
            _this.pageNumber = _this.pageNumber + 1;
            console.log('this.pageNumber', _this.pageNumber);
            var scrollFrom = 0;
            if (_this.prevScrollEle)
                scrollFrom = _this.prevScrollEle.offsetTop;
            var nextPage;
            ...
            var allRows = document.getElementsByClassName('cat-det-row');
            ...
            //new stull
            if (_this.doNotScroll == false)
                _this.prevScrollEle = nextPage;
            if (nextPage && nextPage.offsetTop)
                _this.animate(document.scrollingElement || document.documentElement, "scrollTop", "", scrollFrom, nextPage.offsetTop, 1000, true);
            _this.startPaging();
        }, this.scrollInterval);
    };
    AppComponent.prototype.mainLoopTimer = function () {
        return this._mainLoopTimer;
    };
    AppComponent.prototype.changeInterval = function () {
        var itemsCount = 0;
        this._data[this._menuId].items.forEach(function (_cat) {
